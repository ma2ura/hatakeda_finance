...
g <- ggplot(df) + aes(y = sigma_p, x = mu_p) + geom_line() + geom_point(data = filter(df, label != ""), aes(label = label), size = 2) + coord_flip() + ylim(0, 0.2) + xlim(0, 0.14) +
  labs(title = "投資可能領域のプロット", x = "期待リターン", y = "リスク（標準偏差）") + theme_classic(base_family = "Japan1GothicBBB") # Add labels for clarity
# 資産Aと資産Bの文字を追加し，hiragino Kaku Gothic ProNはMacの場合のフォント指定
g <- g +
  geom_text(aes(label = "資産A"), x = 0.12, y = 0.19, size = 4, family = "Japan1GothicBBB") +
  geom_text(aes(label = "資産B"), x = 0.06, y = 0.13, size = 4, family = "Japan1GothicBBB") +
  geom_text(aes(label = "無相関のケース"), x = 0.09, y = 0.16, size = 4, family = "Japan1GothicBBB") # Specify the font family for consistency
print(g)
```
<!---
...
# g <- g +
#   geom_text(label = "資産A", x = 0.12, y = 0.19, size = 4, family = "Japan1GothicBBB") +
#   geom_text(label = "資産B", x = 0.06, y = 0.13, size = 4, family = "Japan1GothicBBB") +
#   geom_text(label = "無相関のケース", x = 0.09, y = 0.19, size = 4, family = "Japan1GothicBBB")
...
g <- g +
  geom_text(label = "資産A", x = 0.12, y = 0.19, size = 4, family = "Japan1GothicBBB") +
  geom_text(label = "資産B", x = 0.06, y = 0.13, size = 4, family = "Japan1GothicBBB") +
  geom_text(label = "資産C", x = 0.04, y = 0.08, size = 4, family = "Japan1GothicBBB")
