...
```{r setup, filename = "パッケージの読み込みとテーマの設定"}
pacman::p_load(
  tidyverse,
  ggthemes,
  pdftools
...
```{r plot-topix, filename = "TOPIXの動きの確認"}
...
```{r calc-return, filename = "リターンの計算"}
# リターンの計算
# 分母がゼロかどうかを確認してから計算する
df <- df |>
  mutate(
      TOPIX_return = ifelse(lag(TOPIX) == 0, NA, TOPIX / lag(TOPIX) - 1) # TOPIXのリターン
      )
# 作表
df |>
  select(date, TOPIX, TOPIX_return) |>
  head(10) |> # 先頭10行
  knitr::kable(digits = 3, booktabs = TRUE) # 作表
```
...
```{r plot-return, filename = "TOPIXリターンのグラフ化"}
...
```{r plot-coin, filename = "コイン投げの確率分布"}
df <- data.frame(
  coin <- c("HH", "HT","TT"), # 結果の組み合わせ
  p <- c(0.25, 0.5,0.25) # 確率
)
g <- ggplot(df) +
  aes(x = coin, y = p) +
  geom_col() +
  ylim(0,1) + xlab("確率") + ylab("結果")
print(g)
```
